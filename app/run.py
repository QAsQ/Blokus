...
from models.app_utility import success, failure, field_checker, current_time, require_format, generate_register_token, get_email_from_token, token_verify, field_checker_json
...
@field_checker_json(['email'])
def regiester():
    request_json = request.get_json(force=True)

    result = send_register_mail(
        request_json['email'],
        url_head + "/register?token=" +
        generate_register_token(request_json['email']),
        email_config
    )
...
def password_resetter():
    ...
    result = send_reset_mail(
        request_json['email'],
        url_head + "/password_resetter?token=" +
        generate_register_token(request_json['email']),
        email_config
    )
...
@field_checker_json(['email', 'password'])
def confirm():
    request_json = request.get_json(force=True)
...
@field_checker_json(['username', 'email', 'password', 'token'], method='POST')
def users():
...
@field_checker_json(['old_password', 'new_password', 'confirm_password'])
def user(user_id):
...
@field_checker_json(['token', 'password'])
...
@field_checker_json(['battle_name', 'accuracy_time', 'additional_time', 'board_type'], method='POST')
def battles():
    if request.method == 'GET':
        try:
            query = json.loads(request.args.get("query", "{}"))
            sort = json.loads(request.args.get("sort", "[]"))
            start = json.loads(request.args.get(
                'start', "{'start': 0}"))['start']
            limit = json.loads(request.args.get(
...
@field_checker_json(['player_id', 'piece_id', 'position'], method='POST')
...
def next_battle(battle_id):
    ...
    if battle_id not in next_battle_cache:
        ...
        next_battle = BattleFactory.clone_battle(
            current_time(), battle_data, db)
...
def players(battle_id, player_id):
    ...
    if request.method == 'POST':
        result = battle.try_join_player(
            current_time(), player_id, current_user.user_id, current_user.dump())
        if (isinstance(result, str)):
            return failure(result)
        ...
        result = battle.try_remove_player(
            current_time(), player_id, current_user.user_id)
        if (isinstance(result, str)):
            return failure(result)
