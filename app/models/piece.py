...
class Piece:
    ...
    def action_generate(position):
        ...
        result_action = [ [ [ [], [], [] ] for j in range(20) ] for i range(20) ]
        cor_mark = [ [ 0 for j in range(20) ] for i range(20) ]
        for i in range(20):
            for j in range(20):
                if check_legel_bg(i, j, position) == False:
                    continue
                for pos in position:
    ...
    def update_possible_position(self, piece_shape, position, is_same_player):
        for state in range(8):
            for pos in position:
                for act in self.result_action[state][pos[0]][pos[1]][2]:
                    self.possible_position[state][act[0]][act[1]] = 2
            if is_same_player == True:
                for pos in position:
                    for act in self.result_action[state][pos[0]][pos[1]][1]:
                    self.possible_position[state][act[0]][act[1]] = 2
                for pos in position:
                    for act in self.result_action[state][pos[0]][pos[1]][0]:
